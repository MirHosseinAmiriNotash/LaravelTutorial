...
لاراول برای ذخیره‌سازی مدل‌های مرتبط متدهای ساده و قدرتمندی ارائه می‌دهد که به جای تنظیم دستی کلیدهای خارجی، رابطه را مدیریت می‌کنند.

فرض کنید می‌خواهید یک کامنت جدید به یک پست اضافه کنید. به جای اینکه دستی مقدار post_id را در مدل Comment ست کنید، می‌توانید از متد save روی رابطه استفاده کنید:
...
در اینجا ما از متد comments() استفاده کردیم (نه پراپرتی داینامیک comments) تا یک instance رابطه به دست آوریم. سپس متد save به صورت خودکار مقدار post_id را برای ما در مدل Comment ست می‌کند.

اگر نیاز دارید چند مدل مرتبط را همزمان ذخیره کنید، از متد saveMany استفاده می‌کنیم:
...
متدهای save و saveMany مدل‌ها را ذخیره می‌کنند اما به صورت خودکار آن‌ها را به رابطه‌های در حال حاضر لود شده اضافه نمی‌کنند. اگر بلافاصله بعد از ذخیره بخواهید به همه داده‌ها دسترسی داشته باشید، بهتر است از refresh استفاده کنید:
...
اگر می‌خواهید علاوه بر خود مدل، تمام روابط مرتبط با آن نیز ذخیره شوند، از متد `push` استفاده کنید. به عنوان مثال، در اینجا هم پست و هم کامنت‌ها و هم نویسندگان کامنت‌ها ذخیره می‌شوند:
...
اگر بخواهید عملیات ذخیره‌سازی انجام شود ولی هیچ ایونتی (مانند saving یا saved) اجرا نشود، می‌توانید از متد pushQuietly استفاده کنید:

```php
$post->pushQuietly();
```

با این متدها می‌توانید به راحتی مدل‌ها و روابطشان را ذخیره کنید بدون اینکه نیاز باشد کلیدهای خارجی یا ذخیره‌سازی‌های تودرتو را به صورت دستی مدیریت کنید.

### The `create` Method
علاوه بر save و saveMany، می‌توانیم از متد create استفاده کنیم. تفاوت اصلی این است که:  
متد save یک نمونه کامل مدل Eloquent دریافت می‌کند.

متد create یک آرایه ساده از ویژگی‌ها می‌گیرد و خودش مدل را ایجاد و ذخیره می‌کند.
```php 
use App\Models\Post;
...
```
برای ایجاد چند مدل به طور همزمان می‌توان از `createMany` استفاده کرد:
```php
$post = Post::find(1);
...
```
همچنین می‌توانید از متدهای createQuietly و createManyQuietly برای ایجاد داده بدون اجرای رویدادها استفاده کنید:
```php 
$user = User::find(1);
...
```
علاوه بر این، لاراول متدهای دیگری مانند `findOrNew`, `firstOrNew`, `firstOrCreate`, و `updateOrCreate` نیز برای ایجاد یا بروزرسانی مدل‌ها در روابط ارائه می‌دهد.

### Belongs To Relationships
در روابطی که از نوع Belongs To هستند، یعنی زمانی که یک مدل فرزند به یک مدل والد وابسته است، می‌توانیم با استفاده از متدهای خاصی والد را به فرزند نسبت دهیم یا والد را از فرزند جدا کنیم.

اگر بخواهیم یک مدل فرزند را به یک مدل والد جدید متصل کنیم، از متد `associate` استفاده می‌کنیم. این متد به‌طور خودکار مقدار کلید خارجی (foreign key) در مدل فرزند را ست می‌کند.
```php  
...
مدل User دارای یک رابطه belongsTo با مدل Account است.

با استفاده از متد associate، حساب کاربری (Account) شماره 10 به این کاربر اختصاص داده می‌شود.

سپس با ذخیره (save) مدل، تغییر در دیتابیس اعمال می‌شود.


اگر بخواهیم یک مدل والد را از مدل فرزند جدا کنیم، می‌توانیم از متد `dissociate` استفاده کنیم. این متد کلید خارجی را به مقدار null تغییر می‌دهد.
```php 
...
رابطه‌ی account از کاربر جدا می‌شود.

مقدار کلید خارجی مربوط به account_id در جدول users برابر با null خواهد شد.


### Many to Many Relationships
#### Attaching / Detaching
در روابط Many to Many، هر رکورد در یک جدول می‌تواند با چندین رکورد در جدول دیگر مرتبط باشد. این رابطه معمولاً از طریق یک جدول میانی (pivot table) مدیریت می‌شود. لاراول برای مدیریت این نوع رابطه متدهای متنوعی فراهم کرده است.

برای اضافه کردن یک رکورد به جدول میانی (یعنی برقراری ارتباط بین دو مدل)، می‌توان از متد attach استفاده کرد.
```php 
use App\Models\User;
...
کاربری با شناسه 1 را پیدا می‌کنیم.

با استفاده از attach نقش (role) مشخصی را به این کاربر اضافه می‌کنیم.

همچنین می‌توان هنگام اتصال داده‌های اضافی را نیز در جدول میانی ذخیره کرد:
```php  
$user->roles()->attach($roleId, ['expires' => $expires]);
```

برای حذف یک رابطه از جدول میانی می‌توان از متد detach استفاده کرد. این متد رکورد مربوطه را از جدول میانی پاک می‌کند اما خود مدل‌ها در دیتابیس باقی می‌مانند.
```php 
...
$user->roles()->detach();
```
همچنین می‌توان یک آرایه از شناسه‌ها را هم به detach داد:
```php 
$user->roles()->detach([1, 2, 3]);
```
می‌توان هنگام attach کردن چندین نقش، مقادیر اضافی را نیز در جدول میانی ذخیره کرد:
```php  
$user = User::find(1);
...
attach → افزودن رابطه جدید به جدول میانی.

detach → حذف رابطه از جدول میانی بدون حذف رکورد اصلی.

#### Syncing Associations  

لاراول برای مدیریت راحت‌تر روابط Many-to-Many متدهای متنوعی مانند `sync،` `toggle` و `updateExistingPivot` را فراهم کرده است. 

با استفاده از متد `sync` می‌توانیم مجموعه‌ای از آی‌دی‌ها را در جدول واسط (intermediate table) ثبت کنیم. اگر آی‌دی‌هایی در جدول واسط باشند که در این آرایه وجود نداشته باشند، حذف خواهند شد. در نهایت تنها آی‌دی‌های موجود در آرایه در جدول باقی می‌مانند.

```php  
$user->roles()->sync([1, 2, 3]);
```
 در این حالت، تنها نقش‌های با آی‌دی 1، 2 و 3 برای کاربر باقی خواهند ماند.

همچنین می‌توانید مقادیر اضافی برای جدول واسط نیز همراه آی‌دی‌ها ارسال کنید:
```php
$user->roles()->sync([1 => ['expires' => true], 2, 3]);
```
اگر بخواهید مقدار مشخصی برای تمام رکوردهای جدول واسط ثبت شود، می‌توانید از syncWithPivotValues استفاده کنید:
```php  
$user->roles()->syncWithPivotValues([1, 2, 3], ['active' => true]);
```
در صورتی که بخواهید آی‌دی‌های جدید اضافه شوند ولی آی‌دی‌های قبلی حذف نشوند، می‌توانید از syncWithoutDetaching استفاده کنید:
```php 
$user->roles()->syncWithoutDetaching([1, 2, 3]);
```

#### Toggling Associations
متد `toggle` وضعیت اتصال/حذف را برای آی‌دی‌های داده‌شده تغییر می‌دهد:

اگر آی‌دی در جدول واسط وجود داشته باشد، حذف می‌شود.

اگر وجود نداشته باشد، اضافه می‌شود.
```php 
$user->roles()->toggle([1, 2, 3]);
```
همچنین می‌توانید مقادیر اضافی نیز ارسال کنید:
```php 
$user->roles()->toggle([
    1 => ['expires' => true],
    2 => ['expires' => true],
...
برای به‌روزرسانی یک رکورد موجود در جدول واسط می‌توان از متد `updateExistingPivot` استفاده کرد. این متد کلید خارجی مربوط به رکورد واسط و مقادیر جدید را دریافت می‌کند.
```php 
$user = User::find(1);
...
```
  ...
  در این مثال ستون `active` برای نقش مشخص‌شده به‌روزرسانی می‌شود.

## Touching Parent Timestamps (به روزرسانی زمان والد)
گاهی اوقات لازم است هنگام تغییر یک مدل فرزند، فیلد زمان آخرین بروزرسانی (updated_at) در مدل والد هم به‌طور خودکار تغییر کند. این کار مخصوصاً در روابطی مثل belongsTo یا belongsToMany کاربرد دارد.

مثلاً فرض کنید مدل Comment به مدل Post تعلق دارد. اگر کامنت تغییر کند، می‌خواهیم زمان بروزرسانی پست هم تغییر کند. 

برای انجام این کار، در مدل فرزند یک property به نام touches تعریف می‌کنیم و نام رابطه‌هایی که باید هنگام تغییر، زمانشان بروزرسانی شود را داخل آن قرار می‌دهیم:
...
در این مثال هر زمان یک Comment تغییر کند، مقدار updated_at برای مدل Post هم به‌روز می‌شود.

این بروزرسانی فقط زمانی اتفاق می‌افتد که تغییر مدل فرزند از طریق متد Eloquent save انجام شود.  

پس اگر می‌خواهید هنگام تغییر مدل‌های فرزند، مدل والد هم زمان آخرین بروزرسانی جدیدی دریافت کند، از touches استفاده کنید.